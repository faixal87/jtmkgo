...
<article class="enterprise-card min-w-0 rounded-xl border p-4 shadow-sm {{ $notification->read_at ? '' : 'ring-1 ring-[var(--color-accent)]' }}">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2">
                ...
                @endunless
                <h4 class="break-words font-semibold text-[var(--color-text)]">{{ $notification->title }}</h4>
            ...
            <p class="mt-2 break-words text-sm leading-6 text-[var(--color-muted)]">{{ $notification->message }}</p>
