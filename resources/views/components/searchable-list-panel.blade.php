...
<aside {{ $attributes->merge(['class' => 'border-b border-[var(--color-border)] bg-[var(--color-secondary-bg)] lg:border-b-0 lg:border-r']) }}>
    <div class="sticky top-0 z-10 border-b border-[var(--color-border)] bg-[var(--color-secondary-bg)] p-4">
        ...
        <div class="mt-2 flex items-center gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2">
            ...
            <input x-model="{{ $model }}" placeholder="{{ $placeholder }}" class="w-full border-0 bg-transparent p-0 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] focus:ring-0">
        </div>
    </div>
